...
scan_start_port = input("Enter the start port: ").strip()               #.strip() removes whitespace's from start and end of user input.
scan_end_port = input("Enter the end port: ").strip()
port_timeout = 1                                       # One second timeout for each port scan

# Step 2: Verify user input.
def validate_ip_or_hostname(address):     # function to validate IP address or hostname 
    try:
...
log_file_path = "port_results.log"            # Define the log file path in a variable. 
...
try:
    os.chmod(log_file_path, 0o644)  # Owner can read/write, group can read only. 
except:
